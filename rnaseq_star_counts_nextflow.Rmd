...
If testing interactively on cybertron, you will need to request the following resources:

```{bash}
qsub -I -q <queue_name> -P $(voucher info <project_name>) -l select=1:mem=32gb:ncpus=4
```

# Create Demo Dataset

```{r eval=FALSE}
sampler <- ShortRead::FastqSampler("test_data/Day0.Naive_L1Pf7_R1.fastq.gz", 10000)
set.seed(123)
R1 <- ShortRead::yield(sampler)
sampler <- ShortRead::FastqSampler("test_data/Day0.Naive_L1Pf7_R2.fastq.gz", 10000)
set.seed(123)
R2 <- ShortRead::yield(sampler)

ShortRead::writeFastq(R1, file = "test_data/Sample_ID_001_R1.fastq.gz")
ShortRead::writeFastq(R2, file = "test_data/Sample_ID_001_R2.fastq.gz")
```
...
Next include `STAR_ALIGN`  in the same format as `FASTQC` to the nextflow `main.nf` file. And we will define the basic workflow. And example in psuedo-code is below:
...
```{r}
files <- tibble(files = dir("test_data", full.names = T, recursive = T, pattern = "Sample.+.gz")) %>%
  mutate(Read = ifelse(grepl("_[Rr]1.(fastq|fq).gz", files), "r1", "r2")) %>%
  pivot_wider(names_from = Read, values_from = files) %>%
  mutate(
    id = gsub("^(.+)_[Rr][12].(fastq|fq).gz", "\\1", basename(r1)),
    single_end = "false"
  )

files
# write.table(files, file = "test_data/sample_sheet.txt", sep="\t", row.names = FALSE, quote = FALSE)
```
...
In this case, we will use `map` operator on a channel to create our desired `meta` input to run the fastqc module using the "sample_sheet.txt" just created. 
...
The user parameters, such as the file paths to the appropiate genomes, should be included in the `nextflow.config` file as well. This would allow the user to simply fill in the necessary parameters in the config file and then use the same run script (.sh) and the same `main.nf` workflow each time, without having the modify them. 

The HPC requires both a queue and a project code in order to use PBS scheduler. These must be defined in the user params and input into the configs. 

```{bash}
#on cybertron to identify the project codes
voucher
voucher info
voucher project RSC_adhoc
```

**To Do**: Create a function that could create the config file given the appropriate filepaths. Maybe make into a interactive shiny-app to have user input and save the file. 
...
Also, singularity containers on cybertron can be used interactively. Load the singularity module first.  Then, singularity image will need to be pulled.
...
The final profile, like `PBS_Singularity`, would the final profile to execute the fully developed workflow on the HPC. The HPC requires both a queue and a project code in order to use PBS scheduler. These must be defined in the user params and input into the configs. 



```
//Create profiles to easily switch between the different process executors (platforms)
profiles {
