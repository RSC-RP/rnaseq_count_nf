...
```{r eval=TRUE}
knitr::include_graphics("images/nextflow_dsl2.png")
```
...
files
# write.csv(files, file = "test_data/sample_sheet.txt", row.names = FALSE, quote = FALSE)
```
...
```{r}
sra_sample_sheet <- read.csv("test_data/SraRunTable.txt") %>% 
  slice(1:2) %>% 
  mutate(single_end="true") %>% 
  select(id=Run, 
         single_end)

sra_sample_sheet
# write.csv(sra_sample_sheet,"test_data/sra_sample_sheet.txt", quote=FALSE, row.names = FALSE)
```
