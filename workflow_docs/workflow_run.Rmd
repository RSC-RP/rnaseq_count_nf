...
Edit the `nextflow.config` file in any text editor; the example below is in R. You will need to change the project code (use the same one as you used above), and the queue name to be either freeq or paidq. Freeq is sufficient for testing with the workflow's example data provided in the directory `test_data`.
