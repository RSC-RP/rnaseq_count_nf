...
and are located at `/gpfs/shared_data` to share with the SCRI. 
...
```
 ...
 You can also change the [`entry_point`](https://www.nextflow.io/docs/latest/dsl2.html#workflow-entrypoint) for the workflow to run only the index building step using `NFX_ENTRY='star_index'`. You can run only the download step for SRA files by setting `NFX_ENTRY='sra_fastqs'`. Keep the default `NFX_ENTRY='rnaseq_count'` to run the complete pipeline. 
